...
class P3D(nn.Module):
    def __init__(self,
        ...
        self.conv1_custom = nn.Conv3d(self.input_channel,
                                      ...
                                      padding=(0, 3, 3),
                                      bias=False)
        # nn.Conv3d(in_channels,out_chs,kernel_size,stride,padding,dilation)
        # input: (N, C, D, H, W)
        # output: (N, C_out, D_out, H_out, W_out)
        # D_out = (D + 2*pad - dilation*(kernel_size - 1)-1)//stride + 1
        # H_out, W_out  etc.
        ...
        # nn.BatchNorm3d(feature_dimension) batchnorm over batch data of H,W,T
        self.cnt = 0
        self.relu = nn.ReLU(inplace=True)
        ...
        # nn.MaxPool3d(kernel_size, stride, padding, dilation, return_indices)
        # over H, W, T
        # input: (N, C, D, H, W)
        # output:(N, C, D_out, H_out, W_out)
        # D_out = (D + 2*pad - dilation*(kernel_size - 1)-1)//stride + 1
        # H_out, W_out is calculated at the same way with D_out
